    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('image', function ($row) {
                if ($row->image != null) {
                    $asset = asset('storage/banner/' . $row->image);
                } else {
            ...
            ->addColumn('action', function ($row) {
                $editbtn = Gate::any(['access-all']) ? '<a href="javascript:;" id="incomeCategoryEditBtn" onclick="editBanner(' . $row->id . ')"  class="btn btn-sm mx-1 btn-info" ><i class="fas fa-pen-nib"></i></a>' : '';
