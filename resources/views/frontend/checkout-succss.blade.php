    ...
    <div class="container my-5">
        <div class="d-flex justify-content-center">
            <div class="order-success-card shadow-sm text-center">
                <h1 class="mb-3" style="font-size: 30px;color:green;font-weight: 900"><i class="bi bi-check-circle"></i>
                    Order Placed!</h1>
                ...
                <table class="table table-bordered order-summary text-start">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $order->address }}, {{ $order->zip ?? '' }}, {{ $order->country ?? '' }},
                                {{ $order->city ?? '' }}</td>
                        </tr>
                        <tr>
                            <th>Payment Type</th>
                        ...
                        @foreach ($order->orderItems as $item)
                            <tr>
                                ...
                                <td style="border: 1px solid #222">
                                    {{ config('company.currency_symbol') }}{{ number_format($item->price, 2) }}</td>
                                <td style="border: 1px solid #222">
                                    {{ config('company.currency_symbol') }}{{ number_format($item->qty * $item->price, 2) }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <div class="totals mt-4 text-end">
                    ...
                    <h5>Discount: {{ config('company.currency_symbol') }}{{ number_format($order->total_discount, 2) }}
                    </h5>
                    <h5>Shipping Charge:
                        {{ config('company.currency_symbol') }}{{ number_format($order->total_shipping_charge, 2) }}</h5>
                    <h5 class="grand-total">Grand Total:
                        {{ config('company.currency_symbol') }}{{ number_format($order->grand_total, 2) }}</h5>
                </div>

                <a href="{{ route('frontend.home') }}" class="btn btn-primary mt-4"><i class="bi bi-arrow-left-circle"></i>
                    Continue Shopping</a>
            </div>
        </div>
    </div>
